...
def start_inference(opt, exp_name=None):
    logger.info("Setup config, data and model...")

    # MLflow setup
    mlflow.set_experiment("DCASE-Task6")
    ...
    with mlflow.start_run(run_name=exp_name):
        ...
        # Log model path and evaluation split
        mlflow.log_param("model_path", opt.model_path)
        mlflow.log_param("eval_split", opt.eval_split_name)
        ...
        if metrics:
            ...
            # Log all metrics (sanitize metric names for MLflow)
            ...
            for file_path in latest_file_paths:
                if file_path.endswith("_metrics.json"):
                    mlflow.log_artifact(file_path, artifact_path="metrics")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', '-e', type=str, default=None, help='experiment name for MLflow')
    ...
    start_inference(opt, exp_name=args.exp_name)
