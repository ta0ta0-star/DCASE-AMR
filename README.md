...
**Clotho-Moment pre-training and CASTELLA fine-tuning (same recipe as the statistics table “Clotho-Moment pre-training & CASTELLA fine-tuning”)** — run Clotho training, then CASTELLA training with `--resume` pointing at the Clotho run’s best checkpoint (under `results_pretraining/best_checkpoint.pth` by default):
...
Or use the orchestration script (equivalent order):

```
python main.py --two-stage
```

(`--pretrain-config` / `-c` override the YAML paths if needed.)
